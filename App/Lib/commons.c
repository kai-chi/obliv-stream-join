...
//                                "will check in the enclave.");
            ...
            case 'h':
                logger(INFO, "\nThe currently supported list of command line arguments:\n"
                             "* `-a | --alg` - join algorithm name, e.g. `SHJ-L2`, `FK-MERG-L4`, `NFK-JOIN-L3`.\n"
                             "                 See Enclave/Enclave.cpp's sgx_algorithms[] for the full list. Default: `SHJ-L0`\n"
                             "* `-b | --r-batch` - stream R micro-batch size, in tuples.\n"
                             "* `-c | --s-batch` - stream S micro-batch size, in tuples.\n"
                             "* `-d | --dataset` - name of a pre-defined dataset config: `synth-1`, `synth-2`, `tpch-1`.\n"
                             "                     Overrides r/s size, rate, window, batch and fk-join. See App/Lib/commons.c for the exact values.\n"
                             "* `-h` - print this help message\n"
                             "* `-n | --nthreads` - number of enclave threads to use. Default: `1`\n"
                             "* `-p | --r-rate` - stream R arrival rate, tuples/s. Default: `1000`\n"
                             "* `-q | --s-rate` - stream S arrival rate, tuples/s. Default: `1000`\n"
                             "* `-r | --r-size` - number of tuples to generate for stream R. Default: `1000000`\n"
                             "* `-s | --s-size` - number of tuples to generate for stream S. Default: `1000000`\n"
                             "* `-t | --r-path` - load stream R from a file instead of generating it (one integer key per line). Default: none\n"
                             "* `-u | --s-path` - load stream S from a file instead of generating it. Default: none\n"
                             "* `-v | --r-window` - stream R sliding window size, in tuples. Default: `1000000`\n"
                             "* `-w | --s-window` - stream S sliding window size, in tuples. Default: `1000000`\n"
                             "* `-z | --skew` - Zipfian skew factor for stream S's keys (0 = uniform). Default: `0`\n"
                             "\n"
                             "Flags (no argument):\n"
                             "* `--csv` - print results as CSV instead of the default log format. Default: off\n"
                             "* `--self-join` - join stream R against itself instead of R against S. Default: off\n"
                             "* `--fk-join` - required by the FK-*/NFK-JOIN-* algorithms. Default: off\n"
                             "* `--no-sgx` - skip the enclave; only meaningful for `SHJ` (insecure baseline) and `SHJ-L0`. Default: off");
                exit(EXIT_SUCCESS);
                break;
