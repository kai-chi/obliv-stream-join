...
// SHJ-L0: identical control flow to join_st, but every probe/build compares
// AES-DET-encrypted key/payload bytes (row_enc_t) instead of plaintext, so
// matching relies on DET encryption producing identical ciphertext for
// identical plaintext (see DETBucketChainingHashTable).
