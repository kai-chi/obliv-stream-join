...
// Result sink for join_l3: intentionally a no-op. The benchmark measures
// join throughput/latency rather than downstream delivery, and dummy results
// (from padding) must be indistinguishable from real ones at this point, so
// nothing here can branch on whether emit_result was even called.
...
// See SHJ.h for the full description. Per micro-batch: obliviously probe each
// new tuple's degree in the opposite window (WEIGHT via htX->getSize), turn
// those degrees into per-tuple output slot ranges, pad the total request
// count m up to either the batch's real cardinality (padding=false, SHJ-L3)
// or its worst case batchSizeR*batchSizeS (padding=true, SHJ-L4), then run
// oblivious_expansion so every one of the m oblivious probes below touches a
// fixed, input-independent index position.
