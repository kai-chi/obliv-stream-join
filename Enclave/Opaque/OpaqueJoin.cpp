...
#include <ObliviousComputationApproach/threading.h>
#include <pthread.h>
#include <vector>
...
// a slot counts as real only if scan() filled in both sides instead of
// leaving the infinity placeholder
/* Below this many elements per thread a bulk pass isn't worth dispatching. */
#define OPQ_PAR_MIN 65536
...
// The scan carries the last-seen PK tuple forward, which makes it look
// inherently sequential. It isn't: slicing it only needs each slice to know
// which PK tuple was live when it starts. So pass 1 finds the last PK tuple in
// each slice (read-only, every element touched, so still oblivious), a serial
// fixup over just the per-slice results propagates the carry-in, and pass 2
// runs the real scan on all threads at once. Output goes straight into
// preallocated slots instead of push_back, which also drops the vector's
// geometric-growth copies.
struct opq_scan_args {
    table_id_t *t;
    res_type *out;
    size_t begin;
    size_t end;
    row_table_t carry;      // PK live when this slice starts
    row_table_t last_pk;    // last PK tuple inside this slice
    bool has_pk;
    uint32_t matches;
};

static void opq_last_pk_slice(void *args_) {
    struct opq_scan_args *args = (struct opq_scan_args *) args_;
    row_table_t *tuples = args->t->tuples;
    row_table_t last = args->last_pk;
    bool has = false;
    for (size_t i = args->begin; i < args->end; i++) {
        bool is_pk = !tuples[i].table_id;
        last = conditional_select(tuples[i], last, is_pk);
        has |= is_pk;
    }
    args->last_pk = last;
    args->has_pk = has;
...
static void opq_scan_slice(void *args_) {
    struct opq_scan_args *args = (struct opq_scan_args *) args_;
    row_table_t *tuples = args->t->tuples;
    row_table_t inf = {.ts = {.tv_sec = UINT64_MAX, .tv_nsec = UINT64_MAX},
        .key = UINT32_MAX, .payload = UINT32_MAX, .table_id = UINT8_MAX};
    row_table_t t_pk = args->carry;
    res_type *out = args->out;
    uint32_t matches = 0;

    for (size_t i = args->begin; i < args->end; i++) {
        t_pk = conditional_select(t_pk, tuples[i], tuples[i].table_id);
        bool emit_real = tuples[i].table_id == 1 && tuples[i].key == t_pk.key;
        matches += emit_real;
        row_table_t t_r = conditional_select(t_pk, inf, emit_real);
        row_table_t t_s = conditional_select(tuples[i], inf, emit_real);
        out[i] = {t_r, t_s};
    }
...
uint32_t OpaqueJoin::scan(table_id_t *t, std::vector<std::tuple<row_table_t, row_table_t>> &results,
    ...
    row_table_t inf = {.ts = {.tv_sec = UINT64_MAX, .tv_nsec = UINT64_MAX},
        .key = UINT32_MAX, .payload = UINT32_MAX, .table_id = UINT8_MAX};
    size_t n = t->num_tuples;
    size_t base = results.size();
    results.resize(base + n);
    res_type *out = results.data() + base;

    size_t threads = num_threads;
    while (threads > 1 && n / threads < OPQ_PAR_MIN) threads--;

    struct opq_scan_args args[threads];
    ...
    if (threads > 1) {
        for (size_t i = 1; i < threads; i++) {
            work[i] = { .type = THREAD_WORK_SINGLE,
                        .single = { .func = opq_last_pk_slice, .arg = &args[i] } };
            thread_work_push(&work[i]);
        }
        opq_last_pk_slice(&args[0]);
        for (size_t i = 1; i < threads; i++) thread_wait(&work[i]);

        row_table_t carry = inf;
        for (size_t i = 0; i < threads; i++) {
            row_table_t mine = args[i].last_pk;
            bool has = args[i].has_pk;
            args[i].carry = carry;
            carry = conditional_select(mine, carry, has);
        }
    }

    for (size_t i = 1; i < threads; i++) {
        work[i] = { .type = THREAD_WORK_SINGLE,
                    .single = { .func = opq_scan_slice, .arg = &args[i] } };
        thread_work_push(&work[i]);
    }
    opq_scan_slice(&args[0]);
    for (size_t i = 1; i < threads; i++) thread_wait(&work[i]);

    uint32_t matches = 0;
    for (size_t i = 0; i < threads; i++) matches += args[i].matches;
    return matches;
}
...
result_t *OpaqueJoin::l3_l4_join(relation_t *relR, relation_t *relS, joinconfig_t *cfg, bool L3)
{
    ...
    // Worker pool for the whole stream. Without this, every bitonic_sort and
    // every compaction below pushed work onto the queue with nobody to run it
    // and then spun forever in thread_wait -- i.e. FK-SORT hung outright for
    // any --nthreads > 1, which is why it has no multi-threaded numbers.
    int ret;
...
// no-op on purpose - matches are already counted via matches/local_matches,
// so we skip the actual output/I/O here to measure just the join cost.
// uncomment the body below if you need to eyeball individual matches.
