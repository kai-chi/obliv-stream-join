...
// Path-ORAM storage backing the B-Tree index tier. Each B-Tree node lives at
// a random leaf in a binary tree of buckets; a read/write for node N fetches
// the whole path from root to N's assigned leaf into the stash, does the
// access, reassigns N a fresh random leaf, and evicts the stash back down
// paths - the standard Path-ORAM dance. `localStore` (LocalRAMStore) is
// where the buckets actually sit: kept inside the enclave's own memory
// rather than shipped out via OCALLs, per the paper's in-enclave ORAM
// server design for SHJ-L2 (no TEE-client encryption, no ecall/ocall
// round-trips per window operation).
class BTreeORAM {
private:
