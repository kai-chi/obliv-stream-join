...
// The index tier of OBLIWIND: a standard B-Tree (insertNonFull, splitChild,
// borrowFromPrev/Next, merge, fill - textbook CLRS stuff) rewritten so every
// branch is a constant-time select instead of an if, and backed by
// BTreeORAM instead of plain memory, so the sequence and pattern of node
// reads/writes doesn't depend on the key being searched/inserted/removed.
// Nodes are also order-statistic augmented (subtree size counters, see
// BTreeNode::getSubTreeSize) which is what lets getSize()/size(k) work by
// comparing two index positions instead of counting matches directly.
//
// There are several versions of search/insert/remove left in here from
// development (see the comments above each family below) - only one of each
// is actually oblivious and wired up through BTreeOMAP: search2, insert2,
// removeX. The others (search3, insert3, remove2, remove3) are earlier,
// non-oblivious or partially-oblivious attempts kept around for testing/
// debugging - don't use them as a reference for what "the algorithm" does.
class BTree
{
...
public:
    ...
    // search2 is the real one (used by BTreeOMAP::search, i.e. OBLIWIND's
    // search(k,tau)) - touches a fixed number of nodes regardless of where
    // the key is. search3 skips the oblivious accesses entirely and is only
    // there for sanity-checking results against a plain B-Tree.
    ...
    // Finds the smallest stored key >= `key`. Used to walk duplicate keys one
    // at a time (see BTreeHashTable::probe) and to compute size(k).
    BTreeKeyValuePair* searchSucc(BTreeNode *rootNode, uint64_t key);


    // insert2 is the oblivious path actually used (via BTreeOMAP::insert).
    // insert3 is an older non-oblivious version, kept for comparison/testing.
    // Both return the new root index (the root can change on a split).
    ...
    // Three generations of delete: remove3 is plain/non-oblivious, remove2
    // was a first pass at making it oblivious, removeX is the version that's
    // actually wired up (via BTreeOMAP::remove, i.e. OBLIWIND's expire()).
    // Keep remove3/remove2 in mind as historical baggage, not alternatives.
