// See the class comment in BTree.h for the overview and for which of the
// search/insert/remove variants below is the actual oblivious one in use
// (search2, insert2, removeX) versus older non-oblivious attempts kept for
// testing. Not every helper here has its own comment - a lot of this is
// standard B-Tree bookkeeping (split/merge/borrow/fill) translated line by
// line into constant-time form; read it next to a normal B-Tree writeup if
// the branchless style is hard to follow.
#include "BTree.h"
