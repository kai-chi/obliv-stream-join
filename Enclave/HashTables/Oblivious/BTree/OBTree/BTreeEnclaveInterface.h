...
// Despite the name, this isn't an OCALL boundary - everything here runs
// inside the enclave. It just owns the two BTreeOMAP instances (left/right,
// i.e. window R and window S) and gives BTreeHashTable a plain key/value
// call surface instead of exposing the BTree/BTreeOMAP internals directly.
// The paper's point about running OBLIWIND fully in-enclave (no TEE-client
// encryption, no ecall/ocall transitions for window ops) is why this class
// has no untrusted-side counterpart.
