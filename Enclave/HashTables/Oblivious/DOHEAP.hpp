...
public:
    ...
    static void conditional_swap(HeapNode* a, HeapNode* b, int choice) {
        HeapNode tmp = *b;
        for (uint32_t k = 0; k < b->value.size(); k++) {
            b->value[k] = HeapNode::conditional_select(a->value[k], b->value[k], choice);
        }
        for (uint32_t k = 0; k < b->key.id.size(); k++) {
        ...
        for (uint32_t k = 0; k < b->dum.size(); k++) {
        ...
        for (uint32_t k = 0; k < a->key.id.size(); k++) {
...
// subtree_min is the Path Oblivious Heap trick: each bucket caches the
// minimum key anywhere in its subtree, so FindMin doesn't need to touch
// every bucket on a path, just read the root's cached min.
class HeapBucket {
public:
...
// The expiry tier of OBLIWIND: a doubly-oblivious min-heap (a Path
// Oblivious Heap) storing (timestamp -> key) pairs, path-ORAM style, so the
// oldest tuple in the window can be found and popped in O(log N) without
// leaking which key it was via access pattern. BTreeHashTable::deleteOldest
// calls execute(..., op=1) to pop the min and op=2 to insert - see
// DOHEAPEnclaveInterface for the op codes.
class DOHEAP {
private:
...
public:
    DOHEAP(uint32_t maxSize, bytes<Key> key, bool simulation, int isLeft, int useLocalRamStore);
    ~DOHEAP();
    double evicttime = 0;
    int evictcount = 0;
    int readCnt = 0;
    ...
    void dummy();
    // op=1: extract-min (pop the oldest timestamp, return its key).
    // op=2: insert (k = timestamp, v = key).
    // Called through DOHEAPEnclaveInterface::execute_heap_operation.
