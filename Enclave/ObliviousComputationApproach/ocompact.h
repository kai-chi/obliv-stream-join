...
// Oblivious compaction (paper background, Sasy et al.'s O(N log N)
// construction): given an array and a 0/1 marker per element, obliviously
// moves all marked elements to the front (keeping their relative order),
// without the access pattern revealing which or how many were marked. This
// is what OCA/FK-MERG-L3 use to drop dummy tuples after a join (Algorithm 4,
// OCOMPACTION step) and what oshuffle.h's random-split shuffle is built on.
//
// oblivious_compaction handles power-of-two lengths; or_compaction handles
// arbitrary lengths by peeling off a power-of-two-sized chunk first (same
// trick ORShuffle/Sasy et al. use).
...
// The merge step: stitches two already-compacted halves together. Which
// positions get swapped is computed purely from offset/left_marked_count
// (both known ahead of time from the recursion, not from the data), so the
// o_memswap below always touches the same addresses no matter what was
// marked.
template <typename T>
...
// Compacts arr[start, start+length), length a power of two. `M` marks which
// elements should end up first. `offset` lets a caller treat this range as
// part of a bigger logical array that starts `offset` slots earlier (that's
// how or_compaction below stitches a non-power-of-two prefix on).
// Recurses on both halves, counts how many of the left half got marked, then
// swaps ranges to merge. O(N log N) swaps total.
template <typename T>
...
// Same as oblivious_compaction but works for any length, not just powers of
// two: splits off the largest power-of-two chunk as the right side, compacts
// the (smaller, possibly non-power-of-two) remainder on the left with a
// recursive call to itself, then merges the two.
template<typename T>
