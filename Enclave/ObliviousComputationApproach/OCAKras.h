...
// NFK-JOIN-L2 and NFK-JOIN-L3: the non-FK (generic, duplicates-allowed)
// oblivious stream joins. Streaming has no PK/FK structure to lean on here,
// so this is a streaming adaptation of Krastnikov et al.'s static oblivious
// join instead of the OAppend-based FK machinery in OCA.h. Both CLI
// algorithms run the exact same join() below -- Enclave.cpp's
// KRAS_L2_wrapper just pins the batch size to 1 before calling it, while
// KRAS_L3_wrapper uses whatever batch size was configured. So L2 is really
// "L3, one tuple at a time."
//
// Krastnikov's join works by sorting R and S together by join key, working
// out for each key how many R-tuples and S-tuples share it (the "block
// height/width"), then obliviously expanding both sides so the i-th expanded
// R-row lines up with the i-th expanded S-row for that key -- you get the
// full cross product per key without ever branching on the key itself.
//
// To make that work on a sliding window instead of a static table, every
// entry carries a `fresh` flag (arrived with the most recent batch) and a
// `contributes` flag. write_block_sizes() in the .cpp does two extra passes
// over the sorted window (forward, then backward) to mark which entries are
// actually touching a fresh tuple, so oblivious_expand() only expands blocks
// that contain at least one new match. That's what keeps this incremental
// instead of recomputing the whole window's cross product on every batch.
...
class OCAKras {
    ...
    // copies a batch into the window, tagging every new entry `fresh = true`
    // so the marking passes in write_block_sizes() know it just arrived.
    ...
    // ORETIRE for this join: compact out entries that fell out of the window.
    uint32_t retire(window_entry * window, uint32_t oldest_counter);

    // sorts R+S by join key, then does the forward/backward marking passes
    // (contributes flag) plus the usual Krastnikov block-height/width
    // bookkeeping, restricted to blocks that touch a fresh tuple. Returns
    // the number of join results this batch actually needs to produce.
    uint32_t write_block_sizes(window_entry * table);

    // ties the whole thing together: sort, mark, expand, align, emit. This
    // is what join() calls once per batch.
    uint32_t join_windows(std::vector<std::tuple<table_entry, table_entry>> &results);

    // obliviously turns each surviving entry into `weight_func(entry)` many
    // copies laid out contiguously (Krastnikov's oblivious expand), so that
    // after sorting, matching R/S rows for the same key line up index-for-
    // index without ever comparing keys directly.
    template <int (*weight_func)(table_entry e)>
    void oblivious_expand(window_entry * table);

    // helper used by oblivious_expand: routes each entry to its target slot
    // (given by ind_func) via an oblivious compare-and-swap network instead
    // of a data-dependent scatter.
...
public:
    ...
    // main loop: warm up both windows, then per batch append + join_windows()
    // + retire. See the class comment above for how L2 vs L3 pick batch size.
