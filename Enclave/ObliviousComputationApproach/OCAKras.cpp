...
// Krastnikov's original algorithm computes block_height/block_width for
// every key regardless of whether that key changed this batch. We add two
// scans up front to catch only the keys with a fresh match. Rows are sorted
// (key, table_id) so all table-0 (R) rows for a key sit before the table-1
// (S) rows for that key. Forward scan: for each R row, contributes = its own
// fresh flag; by the time we reach the S rows for that key we already know
// whether any of those R rows were fresh, so S.contributes = self-fresh OR
// that. Backward scan does the mirror image for R, OR-ing in whether any
// same-key S row was fresh. End result: contributes is true iff this row or
// some same-key row on the other side is fresh. Everything below this
// (height/width bookkeeping) is stock Krastnikov, just gated on that flag so
// untouched keys don't get re-expanded every batch.
...
// concat R+S, sort by (key, table_id) and mark contributing entries
// (write_block_sizes), re-sort by (table_id, key, payload) to split cleanly
// into t0/t1, then oblivious_expand both sides by their block width/height
// so matching rows land on the same index, and read off output_size pairs.
