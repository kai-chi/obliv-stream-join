...
// Oblivious shuffle (paper Sec. II-A "Oblivious shuffle" / ORSHUFFLE):
// randomly permutes an array without revealing the permutation through
// memory access patterns. Implemented via the recursive random-split
// construction: each partition marks a uniformly random half of its elements
// using SGX hardware randomness (sgx_read_rand), obliviously compacts
// marked-before-unmarked (parcompact.h's orocpar), then recurses
// independently on each half until partitions of size <= 2, which are
// swapped with a single random coin flip. Total cost is O(N log N) oblivious
// compaction work, matching ORSHUFFLE's bound cited in the paper. `length`
// must be a power of two (a requirement inherited from the compaction
// primitive).
...
// Grabs one random bit from the SGX RNG, used for the length==2 base case
// (just flip a coin and maybe swap the pair).
...
// Shuffles arr[start, start+length) in place. Marks length/2 elements at
// random, compacts so marked elements come first, then recurses on each
// half. `length` needs to be a power of two.
template<typename T>
static void oshuffle(void *args_) {
    ...
    /* Mark exactly NUM_TO_MARK elems in our partition. Pulls random coins in
     * batches of MARK_COINS and marks an element if its coin lands above the
     * "remaining slots needed" threshold -- this is what keeps the count
     * exactly num_to_mark instead of just flipping independent coins. */
