...
// FK-EPHI-L2, Algorithm 5. Append the batches (plain append, no sort), shuffle
// both windows obliviously, then build a one-shot multimap on window S and
// probe it with every tuple in window R. The map is thrown away right after
// -- it only ever sees one batch's worth of accesses, so nothing links back
// to past batches. That's what gets us to L2 instead of leaking full access
// patterns like a persistent index would.
