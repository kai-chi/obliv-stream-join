...
block AES::EncryptDET(bytes<Key> key, block plaintext, size_t clen_size, size_t plaintext_size) {
    bytes<IV> iv;
    iv.fill(1);

    block ciphertext = EncryptBlock(key, iv, plaintext, clen_size, plaintext_size);

    // Put deterministic IV at the front of the ciphertext
    ciphertext.insert(ciphertext.end(), iv.begin(), iv.end());
    return ciphertext;
}
