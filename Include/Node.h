...
public:
    ...
    static void conditional_assign(Node* a, Node* b, int choice) {
        a->index = (unsigned long long) Node::conditional_select((long long) b->index, (long long) a->index, choice);
        a->isDummy = Node::conditional_select(b->isDummy, a->isDummy, choice);
        a->pos = (unsigned long long) Node::conditional_select((long long) b->pos, (long long) a->pos, choice);
        for (size_t k = 0; k < b->value.size(); k++) {
            a->value[k] = Node::conditional_select(b->value[k], a->value[k], choice);
        }
        for (size_t k = 0; k < b->dum.size(); k++) {
    ...
    static void conditional_swap(Node* a, Node* b, int choice) {
        Node tmp = *b;
        b->index = (unsigned long long) Node::conditional_select((long long) a->index, (long long) b->index, choice);
        b->isDummy = Node::conditional_select(a->isDummy, b->isDummy, choice);
        b->pos = (unsigned long long) Node::conditional_select((long long) a->pos, (long long) b->pos, choice);
        for (size_t k = 0; k < b->value.size(); k++) {
        ...
        for (size_t k = 0; k < b->key.id.size(); k++) {
            b->key.id[k] = Node::conditional_select(a->key.id[k], b->key.id[k], choice);
        }
        for (size_t k = 0; k < b->leftID.id.size(); k++) {
            b->leftID.id[k] = Node::conditional_select(a->leftID.id[k], b->leftID.id[k], choice);
        }
        for (size_t k = 0; k < b->rightID.id.size(); k++) {
            b->rightID.id[k] = Node::conditional_select(a->rightID.id[k], b->rightID.id[k], choice);
        }

        a->index = (unsigned long long) Node::conditional_select((long long) tmp.index, (long long) a->index, choice);
        a->isDummy = Node::conditional_select(tmp.isDummy, a->isDummy, choice);
        a->pos = (unsigned long long) Node::conditional_select((long long) tmp.pos, (long long) a->pos, choice);
        for (size_t k = 0; k < b->value.size(); k++) {
    ...
    static void conditional_swap(unsigned long long& a, unsigned long long& b, int choice) {
        ...
        b = (unsigned long long) Node::conditional_select((long long) a, (long long) b, choice);
        a = (unsigned long long) Node::conditional_select((long long) tmp, (long long) a, choice);
    }
